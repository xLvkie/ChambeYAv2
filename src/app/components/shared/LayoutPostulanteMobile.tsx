...
import { Home, User, Search, Send, MessageSquare, Star, Bell, Menu } from 'lucide-react';
import { useState } from 'react';

interface LayoutPostulanteMobileProps {
  children: ReactNode;
}

export default function LayoutPostulanteMobile({ children }: LayoutPostulanteMobileProps) {
  ...
  const mainNavItems = [
    ...
    { icon: Search, label: 'Buscar', path: '/postulante/busqueda' },
    { icon: Send, label: 'Postulaciones', path: '/postulante/postulaciones' },
    { icon: User, label: 'Perfil', path: '/postulante/perfil' },
  ];
      ...
      <header className="bg-white border-b border-border px-4 py-3 flex items-center justify-between sticky top-0 z-50">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span className="absolute top-1 right-1 w-5 h-5 bg-accent text-white text-xs font-bold rounded-full flex items-center justify-center">3</span>
          </button>
          <button
      ...
      {menuOpen && (
        <>
          ...
          <div className="fixed top-0 right-0 bottom-0 w-64 bg-white z-50 shadow-xl p-4 space-y-2">
            ...
            <button
              onClick={() => {
                navigate('/postulante/chat');
                setMenuOpen(false);
              }}
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-colors ${
                location.pathname === '/postulante/chat'
            ...
            <button
              onClick={() => {
                navigate('/postulante/calificaciones');
                setMenuOpen(false);
              }}
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-colors ${
                location.pathname === '/postulante/calificaciones'
                  ? 'bg-primary text-white'
                  : 'text-gray-700 hover:bg-gray-100'
              }`}
            >
